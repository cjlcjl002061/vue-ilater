...
import { useChangeMenuWidth } from "../hooks/index";
...
export default {
  state: shallowReactive<StoreState>({
    ...
    themeFontIcolor:defaultSetting.themeFontIcolor,
    ...
    menuWt: 220,
    isRouterAlive:true,
    themeColor: defaultSetting.themeColor,
    activePath:"",
    activeFullPath:"",
  }),
  start(options: any) {
  ...
  setMenuWidth(menuWt: number) {
    this.state.menuWt = menuWt;
  },
  ...
  changeTheme(theme = "dark") {
    ...
    if (theme == "light") {
      el.style.setProperty("--el-color-bg-cjl-menu", "#f2f3f5");
    }
    else {
      el.style.setProperty("--el-color-bg-cjl-menu", "#18a05866");

    }
  },
  changeThemeColor(color = "#ffffff") {
  ...
  addVisitedView(route: any) {
    ...
    return new Promise<any>((resolve) => {
      if (
        ...
        // console.log("this is this.state.visitedView: ",this.state.visitedView)
  ...
  hidden  () { this.state.isRouterAlive = false },
  display () { this.state.isRouterAlive = true },
